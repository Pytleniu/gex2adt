...
def main(model_name, output_file_name, linear_type):
    ...
    config = {
        ...
        "model": {
            "latent_dim": 50,
            "linear_out_size": (linear_labels.shape[1], donor_labels.shape[1]),
            "components_std": 1.,
    ...
    train_dataset = TensorDataset(torch.Tensor(X_gex_train), torch.Tensor(X_adt_train), torch.Tensor(Y_train), torch.Tensor(linear_train), torch.Tensor(donor_labels_train))
    val_dataset = TensorDataset(torch.Tensor(X_gex_val), torch.Tensor(X_adt_val), torch.Tensor(Y_val), torch.Tensor(linear_val), torch.Tensor(donor_labels_val))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default="gmmvae_better_simple_s_and_d", help='Model name')
    parser.add_argument('--output', type=str, default="final_2", help='Name of the output file')
    parser.add_argument('--linear', type=str, default="site", help='Linear type')
