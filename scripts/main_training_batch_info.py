...
def get_mse(model, z_mean, gex_array, adt_array, classifier_labels):
    if not torch.is_tensor(z_mean):
        z_mean = torch.Tensor(z_mean)
    ...
    with torch.no_grad():
        gex_decoded = model.gex_decoder.decode(torch.cat([z_mean, classifier_labels], dim=-1)).cpu()
...
def main(model_name, output_file_name, linear_type):
    ...
    linear_test = linear_labels[test_idx]
    ...
    config = {
        ...
        "model": {
            ...
            "loss_function_weights": (0.449, 0.449, 0.001, 1.0, 0.0, 20.0),
    ...
    gex_decoder = GaussianDecoder(
        latent_dim=config["model"]["latent_dim"]+config["model"]["linear_out_size"],
    ...
    classifier_labels_all = torch.Tensor(linear_labels).to("cuda")
    X_gex_train_tensor = torch.Tensor(X_gex_train).to("cuda")
    classifier_labels_train = torch.Tensor(linear_train).to("cuda")
    X_gex_val_tensor = torch.Tensor(X_gex_val).to("cuda")
    classifier_labels_val = torch.Tensor(linear_val).to("cuda")
    X_gex_test_tensor = torch.Tensor(X_gex_test).to("cuda")
    classifier_labels_test = torch.Tensor(linear_test).to("cuda")
    with torch.no_grad():
        _, _, _, _, _, latent_train = model(X_gex_train_tensor, classifier_labels_train)
        _, _, _, _, _, latent_val = model(X_gex_val_tensor, classifier_labels_val)
        _, _, _, _, _, latent_all = model(X_gex_all_tensor, classifier_labels_all)
        _, _, _, _, _, latent_test = model(X_gex_test_tensor, classifier_labels_test)
    ...
    gex_mse, adt_mse = get_mse(model, latent_all, X_gex, X_adt, classifier_labels_all)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default="gmmvae_better_batch_info", help='Model name')
    parser.add_argument('--output', type=str, default="out_2", help='Name of the output file')
