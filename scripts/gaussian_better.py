...
from gex2adt.decoders.decoder import GaussianDecoder
from gex2adt.encoders.encoder import GaussianEncoder
...
def main(model_name, output_file_name, linear_type):
    ...
    cell_types = cell_types.map(level3_mapping)
    label_encoder = preprocessing.LabelEncoder()
    Y = label_encoder.fit_transform(cell_types)
    ...
    rows, cols = X_gex_train_val.shape
    segment_size = rows // 3
    std_dev = 1
    X_gex_train_val[:segment_size] = np.random.normal(1, std_dev, (segment_size, cols))
    X_gex_train_val[segment_size:2 * segment_size] = np.random.normal(-1, std_dev, (segment_size, cols))
    X_gex_train_val[2 * segment_size:] = np.random.normal(-1, std_dev, (rows - 2*segment_size, cols))

    rows = linear_train_val.shape[0]
    cols = 3
    segment_size = rows // 3
    new_batch_train_val = torch.zeros((rows, cols), dtype=torch.float32)
    new_batch_train_val[:segment_size] = torch.tensor([1, 0, 0], dtype=torch.float32).repeat(segment_size, 1)
    new_batch_train_val[segment_size:2 * segment_size] = torch.tensor([0, 1, 0], dtype=torch.float32).repeat(segment_size, 1)
    new_batch_train_val[2 * segment_size:] = torch.tensor([0, 0, 1], dtype=torch.float32).repeat(rows - 2 * segment_size, 1)
    linear_train_val = new_batch_train_val
    ...
    config = {
        ...
        "model": {
            "latent_dim": 50,
            "linear_out_size": linear_train_val.shape[1],
            "components_std": 1.,
            ...
            "loss_function_weights": (0.449, 0.449, 0.001, 1.0, 0.0, 10.0),
    ...
    gex_decoder = GaussianDecoder(
        latent_dim=config["model"]["latent_dim"],
        layers_dims=config["model"]["gex_decoder_layers_dims"],
        output_dim=config["data"]["gex_dim"]
    )
    adt_decoder = GaussianDecoder(
        latent_dim=config["model"]["latent_dim"],
        layers_dims=config["model"]["adt_decoder_layers_dims"],
        output_dim=config["data"]["adt_dim"]
    )

    model = GMMVAE(
        encoder=encoder,
        gex_decoder=gex_decoder,
        adt_decoder=adt_decoder,
        latent_dim=config["model"]["latent_dim"],
        linear_out_size=config["model"]["linear_out_size"],
    ...
    train_dataset = TensorDataset(torch.Tensor(X_gex_train), torch.Tensor(X_adt_train), torch.Tensor(Y_train), torch.Tensor(linear_train))
    val_dataset = TensorDataset(torch.Tensor(X_gex_val), torch.Tensor(X_adt_val), torch.Tensor(Y_val), torch.Tensor(linear_val))
    ...
    model.eval()
    model = model.to("cuda")
    X_gex_all_tensor = torch.Tensor(X_gex_train_val).to("cuda")
    X_gex_train_tensor = torch.Tensor(X_gex_train).to("cuda")
    X_gex_val_tensor = torch.Tensor(X_gex_val).to("cuda")
    # X_gex_test_tensor = torch.Tensor(X_gex_train_val).to("cuda")
    with torch.no_grad():
        _, _, _, _, _, latent_train = model(torch.Tensor(X_gex_train_tensor))
        _, _, _, _, _, latent_val = model(torch.Tensor(X_gex_val_tensor))
        _, _, _, _, _, latent_all = model(torch.Tensor(X_gex_all_tensor))
        # _, _, _, _, _, latent_test = model(torch.Tensor(X_gex_test_tensor))
    ...
    # REGRESSION METRICS
    batch_regression_metrics = evaluate_regression(latent_train, latent_val, linear_train, linear_val)
    ...
    cell_types_train_val = cell_types.iloc[train_val_idx]
    class_subsample_idx = create_subsample(cell_types_train_val)
    batch_subsample_idx = create_subsample(linear_train_val, sample_size=300)
    latent_proj_sub = latent_projected[class_subsample_idx, :]
    latent_proj_sub_2 = latent_projected[batch_subsample_idx, :]

    class_palette = create_color_palette(cell_types_train_val.unique())
    batches_palette = create_color_palette(((1, 0, 0), (0, 1, 0), (0, 0, 1)))
    ...
    cell_labels_indices = [(label, np.where(cell_types_train_val[class_subsample_idx] == label)) for label in cell_types_train_val.unique()]
    batch_labels_indices = [(label, np.where(torch.all(linear_train_val[batch_subsample_idx] == torch.tensor(label), dim=1))) for label in ((1, 0, 0), (0, 1, 0), (0, 0, 1))]
    ...
    for dim1, dim2 in zip(range(0, no_pcs - 1), range(1, no_pcs)):
        ...
        x_2 = latent_proj_sub_2[:, dim1]
        y_2 = latent_proj_sub_2[:, dim2]
        ...
        plot_labels(ax3, x_2, y_2, batch_labels_indices, "Batches", color_palette=batches_palette)
    ...
    for dim1, dim2 in zip(range(0, no_pcs - 1), range(1, no_pcs)):
        ...
        plot_labels(ax1, x_2, y_2, batch_labels_indices, "Batches", color_palette=batches_palette)
    ...
    gex_mse, adt_mse = get_mse(model, latent_all, X_gex_train_val, X_adt_train_val)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default="gmmvae_better_detached", help='Model name')
    parser.add_argument('--output', type=str, default="gauss", help='Name of the output file')
