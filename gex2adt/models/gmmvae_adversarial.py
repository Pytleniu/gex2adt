...
class GMMVAE(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        optimizer, linear_optim = self.optimizers()
        ...
        linear_pred_detached = self.linear(z_means.detach())

        linear_loss, linear_accuracy = self.linear_loss_function(linear_labels, linear_pred_detached)
        # Learn the linear
        self.manual_train(linear_optim, linear_loss)

        linear_pred = self.linear(z_means)
    ...
    def validation_step(self, val_batch, batch_idx):
        # Sprawdz czy wszystko tutaj jest w walidacyjnym modzie
