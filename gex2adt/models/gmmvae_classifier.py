...
class VAE(pl.LightningModule):
    ...
    def loss_function(self, cell_pred, z_means, cell_labels):
        cell_labels = torch.argmax(cell_labels, dim=1)
        loss = torch.nn.CrossEntropyLoss()(cell_pred, cell_labels)
    ...
    def training_step(self, train_batch, batch_idx):
        gex_true, adt_true, guiding_classes, linear_labels, cell_labels = train_batch
    ...
    def validation_step(self, val_batch, batch_idx):
        gex_true, adt_true, guiding_classes, linear_labels, cell_labels = val_batch
        gex_true, adt_true = gex_true.float(), adt_true.float()
        cell_pred, z_means = self.forward(gex_true)
