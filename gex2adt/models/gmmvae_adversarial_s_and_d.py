...
class GMMVAE(pl.LightningModule):
    def __init__(self, encoder, gex_decoder, adt_decoder, latent_dim, linear_out_size, no_components,
        ...
        self.linear_weight = loss_function_weights[5]

        self.optimizer = optimizer

        self.linear_site = Linear(latent_dim, linear_out_size[0])
        self.linear_donor = Linear(latent_dim, linear_out_size[1])
    ...
    def configure_optimizers(self):
        ...
        linear_optim_site = torch.optim.Adam(
            self.linear_site.parameters(),
            lr=0.0001,
            weight_decay=0.0001
        )
        linear_optim_donor = torch.optim.Adam(
            self.linear_donor.parameters(),
            lr=0.0001,
            weight_decay=0.0001
        )
        return optim, linear_optim_site, linear_optim_donor
    ...
    def loss_function(self, gex_true, adt_true, gex_dist, adt_dist, z_means, z_sample, normal_rv, guiding_classes,
                      gmm, linear_labels_site, linear_pred_site, linear_labels_donor, linear_pred_donor, prefix=""):
        ...
        linear_loss_site, linear_accuracy_site = self.linear_loss_function(linear_labels_site, linear_pred_site)
        linear_loss_donor, linear_accuracy_donor = self.linear_loss_function(linear_labels_donor, linear_pred_donor)
        ...
        loss = self.gex_likelihood_weight * gex_likelihood \
            ...
            - self.linear_weight * linear_loss_site \
            - self.linear_weight * linear_loss_donor
        ...
        metrics = {
            ...
            "loss": loss,
            "linear_loss_site": linear_loss_site,
            "linear_accuracy_site": linear_accuracy_site,
            "linear_loss_donor": linear_loss_donor,
            "linear_accuracy_donor": linear_accuracy_donor,
        }
    ...
    def training_step(self, train_batch, batch_idx):
        optimizer, linear_optim_site, linear_optim_donor = self.optimizers()

        gex_true, adt_true, guiding_classes, linear_labels_site, linear_labels_donor = train_batch
        gex_true, adt_true = gex_true.float(), adt_true.float()

        gex_dist, adt_dist, z_sample, normal_rv, gmm, z_means = self.forward(gex_true)
        linear_pred_site_detached = self.linear_site(z_means.detach())
        linear_pred_donor_detached = self.linear_donor(z_means.detach())

        # Learn the linear
        linear_loss_site, linear_accuracy_site = self.linear_loss_function(linear_labels_site, linear_pred_site_detached)
        self.manual_train(linear_optim_site, linear_loss_site)
        linear_loss_donor, linear_accuracy_donor = self.linear_loss_function(linear_labels_donor, linear_pred_donor_detached)
        self.manual_train(linear_optim_donor, linear_loss_donor)

        linear_pred_site = self.linear_site(z_means)
        linear_pred_donor = self.linear_donor(z_means)

        metrics = self.loss_function(gex_true, adt_true, gex_dist, adt_dist, z_means, z_sample,
                                     normal_rv, guiding_classes, gmm, linear_labels_site, linear_pred_site,
                                     linear_labels_donor, linear_pred_donor)

        # Learn rest of the model
        self.manual_train(optimizer, metrics["loss"])

        self.log_metrics(metrics, prefix='training_step/')

    def validation_step(self, val_batch, batch_idx):
        gex_true, adt_true, guiding_classes, linear_labels_site, linear_labels_donor = val_batch
        gex_true, adt_true = gex_true.float(), adt_true.float()
        ...
        self.validation_step_outputs.append({
            ...
            "linear_accuracy_site": metrics["linear_accuracy_site"],
            "linear_accuracy_donor": metrics["linear_accuracy_donor"]
        })
    ...
    def on_validation_epoch_end(self):
        ...
        avg_linear_accuracy_site = torch.stack([x["linear_accuracy_site"] for x in self.validation_step_outputs]).mean()

        avg_linear_accuracy_donor = torch.stack([x["linear_accuracy_donor"] for x in self.validation_step_outputs]).mean()
        ...
        validation_epoch_metrics = {
            ...
            "avg_linear_accuracy_site": avg_linear_accuracy_site,
            "avg_linear_accuracy_donor": avg_linear_accuracy_donor
        }
