...
class GMMVAE(pl.LightningModule):
    def __init__(self, encoder, gex_decoder, adt_decoder, latent_dim, classifier_out_size, no_components,
    ...
    def forward(self, inputs, batch_labels):
        ...
        gex_dist = self.gex_decoder(torch.cat([z_sample, batch_labels], dim=-1))
        adt_dist = self.adt_decoder(z_sample)

        return gex_dist, adt_dist, z_sample, normal_rv, gmm, z_means

    def loss_function(self, gex_true, adt_true, gex_dist, adt_dist, z_means, z_sample, normal_rv, guiding_classes, gmm, linear_labels):
        entropy = torch.mean(normal_rv.entropy())
        gex_mse, adt_mse = self.compute_mse_losses(gex_true, adt_true, z_means, linear_labels)
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        gex_dist, adt_dist, z_sample, normal_rv, gmm, z_means = self.forward(gex_true, linear_labels)
        metrics = self.loss_function(gex_true, adt_true, gex_dist, adt_dist, z_means, z_sample, normal_rv, guiding_classes, gmm, linear_labels)
    ...
    def compute_mse_losses(self, gex_true, adt_true, z_means, linear_labels):
        gex_pred = self.gex_decoder.decode(torch.cat([z_means, linear_labels], dim=-1))
